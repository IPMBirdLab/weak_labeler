import numpy as np
from VideoToolkit.tools import get_cv_resize_function, \
    bbox_from_mask, rescal_to_image
...
class mean_var_window:
    ...
    def set_new_frame(self, frame):
        if frame.shape != self.shape:
            raise ValueError(f"expected image with shape {self.shape} bot got image with shape {frame.shape}")
    ...
    def get_var_with_mean(self, mean):
        ...
        if mean.shape != self.shape:
            raise ValueError(f"expected image with shape {self.shape} bot got image with shape {mean.shape}")
...
class StatBS:
    ...
    def get_uncertainty_maps(self, frame):
        ...
        certain_back[np.where(
                np.abs(frame.astype(np.float32) - self.long_mean) < (1 * np.sqrt(self.long_var))
            )] = 1
